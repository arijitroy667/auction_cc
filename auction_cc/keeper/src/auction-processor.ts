...
function getChainConfig(chainIdentifier: string | number) {
    ...
    console.log(`[DEBUG] Available chains:`, Object.keys(CONFIG.chains));
    ...
    for (const [intentId, auctionData] of allAuctions.entries()) {
        try {
            ...
            if (!winner || highestBid < reservePrice) {
                console.log(`   - No valid bids met the reserve price (${ethers.formatUnits(reservePrice, 6)}). Returning NFT to seller...`);
                try {
                    const cancelTx = await auctionHub.connect(keeperWallet).cancelAuction(intentId);
                    await cancelTx.wait();
                    console.log(`   - NFT returned to seller. Tx: ${cancelTx.hash}`);
