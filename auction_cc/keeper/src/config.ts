...
export const CONFIG = {
  ...
  chains: {
    sepolia: {
      ...
      auctionHubAddress: "0xEF4910C0453EE856465521D39575Bc64c37A479b",
      bidManagerAddress: "0x569a6960BedB3c45cad89225e04a0cE5143A5239",
    },
    polygonMumbai: {
      id: 80001,
      name: "polygonMumbai",
...
export type SupportedToken = typeof CONFIG.supportedTokens[number];

// Utility functions for token handling
export const getTokenAddress = (chainId: number, token: SupportedToken): string => {
  const chainTokens = CONFIG.tokenAddresses[chainId as keyof typeof CONFIG.tokenAddresses];
  if (!chainTokens) {
...
export const isSupportedToken = (token: string): token is SupportedToken => {
  return CONFIG.supportedTokens.includes(token as SupportedToken);
};

export const isSupportedChain = (chainId: number): boolean => {
  return Object.values(CONFIG.chains).some(chain => chain.id === chainId);
};

export const getTokenSymbolByAddress = (chainId: number, address: string): SupportedToken | null => {
  const chainTokens = CONFIG.tokenAddresses[chainId as keyof typeof CONFIG.tokenAddresses];
  if (!chainTokens) return null;
  
  for (const [symbol, tokenAddress] of Object.entries(chainTokens)) {
    if (tokenAddress.toLowerCase() === address.toLowerCase()) {
      return symbol as SupportedToken;
    }
  }
  return null;
};
