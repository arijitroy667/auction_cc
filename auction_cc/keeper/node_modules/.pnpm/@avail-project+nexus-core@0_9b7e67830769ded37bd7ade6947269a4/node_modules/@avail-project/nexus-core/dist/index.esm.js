import { formatBalance, parseUnits, formatUnits, isValidAddress, truncateAddress, chainIdToHex, hexToChainId, getMainnetTokenMetadata, getTestnetTokenMetadata, getTokenMetadata, getChainMetadata, formatTokenAmount, formatTestnetTokenAmount, logger as logger$m, CHAIN_METADATA, CaliburSBCTypes, SwapMode, SUPPORTED_CHAINS, extractErrorMessage, validateContractParams, encodeContractCall, getTransactionHashWithFallback, waitForTransactionReceipt, getBlockExplorerUrl, getTokenContractAddress, TOKEN_METADATA, NEXUS_EVENTS } from './commons';
export * from './commons';
export { CHAIN_METADATA, DESTINATION_SWAP_TOKENS, MAINNET_CHAINS, NEXUS_EVENTS, SUPPORTED_CHAINS, TESTNET_CHAINS, TESTNET_TOKEN_METADATA, TOKEN_CONTRACT_ADDRESSES, TOKEN_METADATA } from './commons';
import { hexToBigInt, formatEther, InternalRpcError, UserRejectedRequestError, SwitchChainError, maxUint256, hexToBytes, pad, parseSignature, createPublicClient, http, fallback, getContract, bytesToHex, encodeFunctionData, decodeFunctionData, toBytes, toHex, getAbiItem, encodeAbiParameters, keccak256, hashMessage, bytesToNumber, bytesToBigInt, concat, webSocket, serializeTransaction, createWalletClient, custom, publicActions, parseUnits as parseUnits$1, erc20Abi, formatUnits as formatUnits$1, encodePacked, hexToNumber } from 'viem';
import { OmniversalChainID, Universe, ChaindataMap, PermitVariant, ERC20ABI, PermitCreationError, createCosmosClient, MsgRefundReq, MsgRefundReqResponse, MsgCreateRequestForFundsResponse, Environment, EVMVaultABI, ArcanaVault, MsgDoubleCheckTx, DepositVEPacket, GrpcWebImpl, QueryClientImpl, OmniversalRFF, MsgCreateRequestForFunds, getVaultContractMap, msgpackableAxios, LiFiAggregator, BebopAggregator, CurrencyID as CurrencyID$1, autoSelectSources, liquidateInputHoldings, determineDestinationSwaps, destinationSwapWithExactIn, createCosmosWallet } from '@arcana/ca-common';
import Decimal from 'decimal.js';
import { CHAIN_IDS, hexlify, arrayify, bn, OutputType, Account, Provider, Address, BN, randomBytes, UTXO_ID_LEN } from 'fuels';
...
import { toHex as toHex$1 } from 'viem/utils';
import { retry, orderBy } from 'es-toolkit';
...
class NexusUtils {
    constructor(adapter, isReady) {
        ...
        this.truncateAddress = truncateAddress;
        this.chainIdToHex = chainIdToHex;
        this.hexToChainId = hexToChainId;
        this.getMainnetTokenMetadata = getMainnetTokenMetadata;
        this.getTestnetTokenMetadata = getTestnetTokenMetadata;
        this.getTokenMetadata = getTokenMetadata;
        this.getChainMetadata = getChainMetadata;
        this.formatTokenAmount = formatTokenAmount;
        this.formatTestnetTokenAmount = formatTestnetTokenAmount;
    }
    ensureInitialized() {
        if (!this.isReady()) {
...
class BackendSimulationClient {
    ...
    async isChainSupported(chainId) {
        try {
            ...
            logger$m.warn(`Error checking chain support for ${chainId}:`, error);
            return false;
        }
    }
    /**
    ...
    async getSupportedChains() {
        try {
            ...
            logger$m.warn('Error fetching supported chains:', error);
            return null;
        }
    }
    /**
    ...
    async getServiceStatus() {
        try {
            ...
            logger$m.warn('Error fetching service status:', error);
            return null;
        }
    }
    /**
    ...
    async healthCheck() {
        try {
            ...
            logger$m.warn('Error performing health check:', error);
            return null;
        }
    }
    /**
    ...
    async testConnection() {
        try {
            ...
            logger$m.warn('Connection test failed:', error);
            return false;
        }
    }
    /**
    ...
    async getServiceInfo() {
        try {
            ...
            logger$m.warn('Error getting service info:', error);
            return {
                healthy: false,
                configured: false,
                supportedChains: 0,
    ...
    async simulate(request) {
        try {
            ...
            logger$m.error('Simulation API error:', error);
            return {
                gasUsed: '0x0',
                gasPrice: '0x0',
                success: false,
    ...
    async getCurrentGasPrice(chainId) {
        try {
            ...
            logger$m.warn('Failed to fetch current gas price, using fallback:', error);
    ...
    async simulateBundle(request) {
        try {
            logger$m.info('DEBUG simulateBundle - request:', JSON.stringify(request, null, 2));
            ...
            logger$m.info('DEBUG simulateBundle - backend response:', result);
            // Fetch current gas price via RPC
            const currentGasPrice = await this.getCurrentGasPrice(request.chainId);
            logger$m.info('DEBUG - Raw gas price from RPC (wei):', currentGasPrice.toString());
            logger$m.info('DEBUG - Gas price in gwei:', (Number(currentGasPrice) / 1e9).toFixed(2));
            logger$m.info('DEBUG - Chain ID:', request.chainId);
            ...
            const transformedResults = result.data.map((item, index) => {
                ...
                logger$m.info('DEBUG - Gas used (units):', gasUsed.toString());
                const gasCostWei = gasUsed * currentGasPrice;
                logger$m.info('DEBUG - Gas cost (wei):', gasCostWei.toString());
            ...
            logger$m.info('DEBUG simulateBundle - transformed response:', {
            ...
            logger$m.error('Bundle simulation API error:', error);
            return {
                success: false,
...
const ErrorUserDeniedIntent$1 = new UserRejectedRequestError(new Error("User denied intent."));
const ErrorUserDeniedAllowance = new UserRejectedRequestError(new Error("User denied allowance."));
const ErrorInsufficientBalance$1 = new InternalRpcError(new Error("Insufficient balance."));
new InternalRpcError(new Error("Error while building intent."));
...
const getSourcesAndDestinationsForRFF = (intent, chainList, destinationUniverse) => {
    ...
    if (intent.destination.gas != 0n) {
        if (isNativeAddress$1(intent.destination.universe, intent.destination.tokenContract)) {
            ...
            destinations.push({
                tokenAddress: convertTo32BytesHex(destinationUniverse === Universe.FUEL ? FUEL_BASE_ASSET_ID : ZERO_ADDRESS),
                universe: intent.destination.universe,
...
const createRFFromIntent = async (intent, options, destinationUniverse) => {
    ...
    for (const universe of universes) {
        ...
        if (universe === Universe.FUEL) {
            ...
            const { requestHash, signature } = await createRequestFuelSignature(options.chainList.getVaultContractAddress(CHAIN_IDS.fuel.mainnet), options.fuel.provider, options.fuel.connector, omniversalRFF.asFuelRFF());
            signatureData.push({
            ...
            if (input.token && input.amount && input.chainId) {
                ...
                if (input.chainId === CHAIN_IDS.fuel.mainnet) {
                    if (this.fuelAccount) {
                        ...
                        '0xE78655DfAd552fc3658c01bfb427b9EAb0c628F54e60b54fDA16c95aaAdE797A', bn(bridgeAmount.toString()), token.contractAddress);
...
const simulate = async (tx, address, provider, chainList) => {
    ...
    const tokens = outputs
        .map((o) => {
        const token = chainList.getTokenByAddress(CHAIN_IDS.fuel.mainnet, o.assetId);
        if (!token)
            return null;
        return {
...
const getFuelProvider = (getBalances, address, chain) => {
    return new (class Provider$1 extends Provider {
        ...
        async assembleTx(params) {
            ...
            for (const a of assetIdsOnFuel) {
                ...
                const asset = balances.find((asset) => asset.breakdown.find((b) => equalFold(b.contractAddress, hexlify(a)) && b.universe === Universe.FUEL));
                const chainAsset = asset?.breakdown.find((b) => equalFold(b.contractAddress, hexlify(a)) && b.universe === Universe.FUEL);
...
const vscSBCTx = async (input, vscDomain) => {
    const ops = [];
    const connection = connect(new URL('/api/v1/create-sbc-tx', getVSCURL(vscDomain, 'wss')).toString());
    try {
        await connection.connected();
        connection.socket.send(pack(input));
        let count = 0;
...
const packERC20Approve = (spender, amount = maxUint256) => {
...
const getAnkrBalances = async (walletAddress, chainList, removeTransferFee = false) => {
    const publicClients = {};
    const res = await axios.post('https://rpc.ankr.com/multichain/269e541dd5773dac3204831e29b9538284dd3e9591d2b7cb2ac47d85eae213b9/', {
...
const getTokenDecimals = (chainID, contractAddress) => {
    ...
    const token = cData.find((c) => equalFold(toHex$1(contractAddress), c.TokenContractAddress));
    if (!token) {
        throw new Error(`token not found: ${toHex$1(contractAddress)}`);
    }
    return {
...
const createBridgeRFF = async ({ config, input, output, }) => {
    ...
    const createRFF = async () => {
        ...
        omniversalRFF.protobufRFF.sources.map((s) => {
            doubleCheckTxMap[bytesToNumber(s.chainID)] = createDoubleCheckTx(s.chainID, config.cosmos, intentID, config.network.COSMOS_URL);
        });
        return {
...
const _exactOutRoute = async (input, params) => {
    ...
    const getDDS = async () => {
        ...
        if (!equalFold(input.toTokenAddress, dstChainCOTAddress)) {
            dds = await determineDestinationSwaps(userAddressInBytes, null, dstOmniversalChainID, {
                amount: BigInt(input.toAmount),
    ...
    if (sourceSwapsRequired) {
        sourceSwaps = await autoSelectSources(userAddressInBytes, balances.map((balance) => ({
    ...
    console.log({
        dstChainCOTBalance,
        inequality: new Decimal(dstChainCOTBalance?.amount ?? 0).lt(dstSwapInputAmountInDecimal.add(fees)),
    });
    ...
    if (new Decimal(dstChainCOTBalance?.amount ?? 0n).lt(dstSwapInputAmountInDecimal.add(fees))) {
        ...
        if (new Decimal(dstChainCOTBalance?.amount ?? 0)
...
const _exactInRoute = async (input, params) => {
    ...
    const getDDS = async () => {
        ...
        if (!equalFold(input.toTokenAddress, dstChainCOTAddress)) {
            dds = await destinationSwapWithExactIn(userAddressInBytes, dstOmniversalChainID, mulDecimals(dstSwapInputAmountInDecimal, dstChainCOT.decimals), convertTo32Bytes(input.toTokenAddress), params.aggregators, dstChainCOT.currencyID);
        }
        const createdAt = Date.now();
        let dstEOAToEphTx = null;
...
class BaseRequest {
    ...
    async processRFF(intent) {
        ...
        for (const [i, s] of sources.entries()) {
            ...
            else if (s.universe === Universe.ETHEREUM && isNativeAddress$1(s.universe, s.tokenAddress)) {
                const chain = this.input.chainList.getChainByID(Number(s.chainID));
                if (!chain) {
                    throw new Error('chain not found');
                }
        ...
        try {
            for (const source of input) {
                ...
                if (currency.permitVariant === PermitVariant.Unsupported) {
                    ...
                    const signed = parseSignature(await signPermitForAddressAndValue(currency, this.input.evm.client, publicClient, account, vc, source.amount));
                    this.markStepDone(ALLOWANCE_APPROVAL_REQ(source.chainID));
...
class NativeTransfer extends BaseRequest {
    ...
    async simulateTx() {
        ...
        const amount = hexToBigInt(value ?? `0x00`);
...
const swap = async (input, options, COT = CurrencyID$1.USDC) => {
    ...
    const dstSwapHandler = new DestinationSwapHandler({ ...destinationSwap, getDDS: swapRoute.getDDS }, dstTokenInfo, {
        ...
        amount: input.mode === SwapMode.EXACT_OUT ? input.data.toAmount : destinationSwap.outputAmount,
    }, opt);
...
class CA {
    constructor(config = { debug: false, network: 'testnet' }) {
        this._caEvents = new SafeEventEmitter();
            ...
            if (this._initStatus === INIT_STATUS.CREATED) {
                ...
                try {
                    ...
                    if (!this._isArcanaProvider) {
                        __classPrivateFieldSet(this, _CA_cosmosWallet, await this._createCosmosWallet(), "f");
    ...
    async getCommonSwapParams(options) {
        return {
            ...
            chainList: this.chainList,
            address: {
                cosmos: (await __classPrivateFieldGet(this, _CA_cosmosWallet, "f").getAccounts())[0].address,
                eoa: (await this._evm.client.getAddresses())[0],
                ephemeral: __classPrivateFieldGet(this, _CA_ephemeralWallet, "f").address,
            },
            wallet: {
                cosmos: __classPrivateFieldGet(this, _CA_cosmosWallet, "f"),
                ephemeral: __classPrivateFieldGet(this, _CA_ephemeralWallet, "f"),
    ...
    async _setFuelConnector(connector) {
        ...
        const modProvider = getFuelProvider(this._getUnifiedBalances.bind(this), address, this.chainList.getChainByID(CHAIN_IDS.fuel.mainnet));
    ...
    async _checkPendingRefunds() {
        ...
        try {
            await refundExpiredIntents(account, this._networkConfig.COSMOS_URL, __classPrivateFieldGet(this, _CA_cosmosWallet, "f"));
    ...
    async _createCosmosWallet() {
        ...
        __classPrivateFieldSet(this, _CA_ephemeralWallet, privateKeyToAccount(`0x${pvtKey.padStart(64, '0')}`), "f");
...
class TransactionService {
    ...
    async estimateTransactionGas(provider, transactionParams) {
        logger$m.info('TransactionService - Starting gas estimation...');
        logger$m.info('TransactionService - Transaction params:', {
        ...
        try {
            ...
            logger$m.info('TransactionService - Gas estimation successful:', {
            ...
            try {
                ...
                logger$m.info('TransactionService - Gas cost estimation:', {
                ...
                logger$m.warn('TransactionService - Failed to get gas price:', gasPriceError);
            }
            return {
                success: true,
                gasEstimate,
            ...
            logger$m.error('TransactionService - Gas estimation failed:', gasEstimateError);
            ...
            if (gasEstimateError && typeof gasEstimateError === 'object') {
                if ('data' in gasEstimateError && gasEstimateError.data) {
                    logger$m.error('TransactionService - Gas estimation revert data:', gasEstimateError.data);
                ...
                if ('message' in gasEstimateError && gasEstimateError.message) {
                    ...
                    logger$m.error('TransactionService - Gas estimation error message:', errorMessage);
    ...
    async sendTransaction(provider, fromAddress, contractAddress, encodedData, value, options) {
        ...
        try {
            ...
            if (this.enableGasEstimation) {
                logger$m.info('TransactionService - Performing pre-execution gas estimation...');
                const gasEstimation = await this.estimateTransactionGas(provider, transactionParams);
                if (!gasEstimation.success) {
                    logger$m.error('TransactionService - Pre-execution gas estimation failed:', gasEstimation.error);
                    if (gasEstimation.revertReason) {
                        logger$m.warn(`TransactionService - Transaction will likely fail: ${gasEstimation.revertReason}`);
                        throw new Error(`Transaction simulation failed: ${gasEstimation.revertReason}`);
                    }
                }
                else {
                    logger$m.info('TransactionService - Gas estimation completed successfully:', {
                ...
                logger$m.info('TransactionService - Gas estimation disabled, proceeding with transaction');
            }
            logger$m.info('TransactionService - Sending transaction...');
            ...
            logger$m.info('TransactionService - Transaction sent successfully:', {
    ...
    async handleTransactionConfirmation(provider, transactionHash, options, chainId) {
        ...
        try {
            ...
            if (!receiptResult.success) {
                logger$m.warn(`Failed to get transaction receipt: ${receiptResult.error}`);
                return {};
            }
            return {
            ...
            logger$m.warn(`Receipt retrieval failed: ${extractErrorMessage(error, 'receipt retrieval')}`);
            return {};
        }
    }
    /**
    ...
    decimals = 18) {
        ...
        try {
            // Perform gas estimation if enabled
            if (this.enableGasEstimation) {
                logger$m.info('TransactionService - Performing gas estimation for native token transfer...');
                const gasEstimation = await this.estimateTransactionGas(provider, transactionParams);
                if (!gasEstimation.success) {
                    logger$m.error('TransactionService - Gas estimation failed for native token transfer:', gasEstimation.error);
                    throw new Error(`Native token transfer gas estimation failed: ${gasEstimation.error}`);
                }
                logger$m.info('TransactionService - Native token transfer gas estimation successful:', {
                    gasEstimate: gasEstimation.gasEstimate,
                    estimatedCost: gasEstimation.estimatedCostEth,
                });
            }
            logger$m.info('TransactionService - Sending native token transfer...');
            const response = await provider.request({
                method: 'eth_sendTransaction',
                params: [transactionParams],
            });
            const transactionHash = getTransactionHashWithFallback(provider, response);
            logger$m.info('TransactionService - Native token transfer sent successfully:', transactionHash);
            return transactionHash;
        }
        catch (error) {
            logger$m.error('TransactionService - Native token transfer failed:', error);
            throw new Error(`Native token transfer failed: ${extractErrorMessage(error, 'native transfer')}`);
        }
    }
    /**
    ...
    decimals = 18) {
        ...
        try {
            ...
            // Perform gas estimation if enabled
            if (this.enableGasEstimation) {
                logger$m.info('TransactionService - Performing gas estimation for ERC20 transfer...');
                const gasEstimation = await this.estimateTransactionGas(provider, transactionParams);
                if (!gasEstimation.success) {
                    logger$m.error('TransactionService - Gas estimation failed for ERC20 transfer:', gasEstimation.error);
                    if (gasEstimation.revertReason) {
                ...
                logger$m.info('TransactionService - ERC20 transfer gas estimation successful:', {
                    gasEstimate: gasEstimation.gasEstimate,
                    estimatedCost: gasEstimation.estimatedCostEth,
                });
            }
            logger$m.info('TransactionService - Sending ERC20 transfer...');
            const response = await provider.request({
                method: 'eth_sendTransaction',
                params: [transactionParams],
            });
            const transactionHash = getTransactionHashWithFallback(provider, response);
            logger$m.info('TransactionService - ERC20 transfer sent successfully:', transactionHash);
            return transactionHash;
        }
        catch (error) {
            logger$m.error('TransactionService - ERC20 transfer failed:', error);
...
class ApprovalService {
        ...
        try {
            ...
            try {
                ...
                if (tokenApproval.amount.includes('.')) {
                    ...
                    amountInWei = parseUnits$1(tokenApproval.amount, decimals);
                }
                else {
                ...
                throw new Error(`Failed to parse amount ${tokenApproval.amount} for ${tokenApproval.token}: ${extractErrorMessage(error, 'amount parsing')}`);
            }
            ...
            throw new Error(`Failed to check approval for ${tokenApproval.token}: ${extractErrorMessage(error, 'approval check')}`);
        }
    }
    /**
    ...
    async ensureContractApproval(tokenApproval, spenderAddress, chainId, waitForConfirmation = false, approvalBufferBps) {
        try {
            ...
            const humanReadableAmount = formatUnits$1(requiredAmountWithBuffer, tokenDecimals);
            logger$m.info('DEBUG approval - Human readable amount for MetaMask:', {
                humanReadableAmount,
                token: tokenApproval.token,
            });
            // Convert back to wei for the transaction
            const finalApprovalAmount = parseUnits$1(humanReadableAmount, tokenDecimals);
            ...
            if (waitForConfirmation) {
                try {
                    ...
                    logger$m.warn('DEBUG approval - Confirmation failed:', confirmationError);
                    return {
                        transactionHash,
                        wasNeeded: true,
                        confirmed: false,
            ...
            logger$m.error('DEBUG approval - Error:', error);
            return {
                wasNeeded: true,
...
class SimulationEngine {
    ...
    async simulateWithStateSetup(params) {
        ...
        try {
            ...
            logger$m.info('DEBUG SimulationEngine - Starting enhanced simulation with full context:', {
                user,
                tokenRequired,
                amountRequired,
                chainId,
            ...
            logger$m.info('DEBUG SimulationEngine - Balance check result:', balanceCheck);
            ...
            logger$m.info('DEBUG SimulationEngine - Generated steps:', steps.length);
            // Step 3: Execute multi-step simulation
            const result = await this.executeBatchSimulation(steps, chainId);
            logger$m.info('DEBUG SimulationEngine - Simulation complete:', {
            ...
            logger$m.error('DEBUG SimulationEngine - Simulation failed:', error);
            return this.createFailedResult(`Enhanced simulation failed: ${extractErrorMessage(error, 'simulation')}`);
        }
    }
    /**
    ...
    async checkUserBalance(user, token, chainId, requiredAmount) {
        try {
            ...
            logger$m.warn(`Failed to check balance for ${token} on chain ${chainId}:`, error);
            return {
                balance: '0',
                sufficient: false,
    ...
    getBalanceStorageSlot(token, chainId) {
        ...
        if (!chainMapping) {
            logger$m.warn(`Unsupported chain ${chainId}, falling back to defaults`);
            // Fallback defaults based on most common patterns
            return token === 'USDC' ? 9 : token === 'USDT' ? 2 : 0;
        }
        const slot = chainMapping[token];
        if (slot === undefined) {
            logger$m.warn(`Token ${token} not supported on chain ${CHAIN_METADATA[chainId]?.name}, falling back to defaults`);
            return token === 'USDC' ? 9 : token === 'USDT' ? 2 : 0;
        }
        logger$m.info(`Using storage slot ${slot} for ${token} on chain ${chainId}`);
        return slot;
    }
    /**
    ...
    async generateStateOverrides(user, token, requiredAmount, chainId) {
        try {
            ...
            const userBalanceSlot = keccak256(encodePacked(['address', 'uint256'], [user, BigInt(balanceSlot)]));
            // Convert amount to hex with proper padding
            const amountHex = `0x${BigInt(requiredAmount).toString(16).padStart(64, '0')}`;
            logger$m.info(`Generating state override for ${token} on chain ${chainId}: slot=${balanceSlot}, storageKey=${userBalanceSlot}`);
            return {
                [tokenAddress]: {
                    storage: {
            ...
            logger$m.error('Error generating state overrides:', error);
            throw error;
        }
    }
    /**
    ...
    async generateSimulationSteps(params) {
        ...
        logger$m.info('DEBUG generateSimulationSteps - Balance analysis:', {
        ...
        logger$m.info('Token metadata:', { meta: TOKEN_METADATA, tokenRequired });
        const decimals = TOKEN_METADATA[tokenRequired]?.decimals ?? 6;
        const userFriendlyAmount = formatUnits$1(BigInt(amountRequired), decimals);
        logger$m.info('DEBUG SimulationEngine - Amount conversion:', {
            microUnits: amountRequired,
            decimals,
            userFriendly: userFriendlyAmount,
        });
        // Call the buildFunctionParams with user-friendly amount
        logger$m.info('DEBUG SimulationEngine - Calling buildFunctionParams with:', {
            tokenRequired,
            userFriendlyAmount,
        ...
        logger$m.info('DEBUG SimulationEngine - buildFunctionParams result:', {
            functionParams,
            value,
        ...
        if (tokenRequired !== 'ETH' && contractCall.tokenApproval) {
            ...
            logger$m.info('DEBUG SimulationEngine - Approval step preparation:', {
                tokenRequired,
                amountRequired,
        ...
        logger$m.info('DEBUG SimulationEngine - Execute step preparation:', {
            encodedData: encodingResult.data,
        ...
        logger$m.info('DEBUG SimulationEngine - Final steps generated:', {
    ...
    async executeBatchSimulation(steps, chainId) {
        ...
        logger$m.info(`DEBUG executeBatchSimulation - Starting bundle simulation with ${steps.length} steps`);
        ...
        for (const step of steps) {
            ...
            logger$m.info(`DEBUG executeBatchSimulation - Prepared step: ${step.stepId} (${step.type})`);
        }
        try {
            ...
            logger$m.info('DEBUG executeBatchSimulation - Sending bundle request');
            ...
            const executedSteps = bundleResult.results.map((result) => {
                ...
                logger$m.info(`DEBUG executeBatchSimulation - Step ${result.stepId} completed:`, {
                    gasUsed: result.gasUsed,
                });
                return {
                    stepId: result.stepId,
            ...
            logger$m.error('Bundle simulation error:', error);
            return this.createFailedResult(`Bundle simulation failed: ${extractErrorMessage(error, 'bundle simulation')}`);
        }
    }
    /**
...
class ExecuteService {
    ...
    async simulateExecute(params) {
        try {
            ...
            const gasUsedDecimal = hexToNumber(simulationResult.gasUsed);
            let gasCostEth;
            try {
                ...
                logger$m.warn('Failed to fetch gas price during simulation cost calc:', gpErr);
            }
            return {
    ...
    async simulateExecuteEnhanced(params) {
        try {
            ...
            logger$m.debug('DEBUG ExecuteService - Full params received:', {
                functionName: params.functionName,
                contractAddress: params.contractAddress,
            ...
            logger$m.debug('DEBUG ExecuteService - Final enhanced simulation decision:', shouldUseEnhancedSimulation);
    ...
    shouldUseEnhancedSimulation(params) {
        ...
        logger$m.debug('DEBUG shouldUseEnhancedSimulation - Decision:', {
    ...
    async runEnhancedSimulation(params) {
        try {
            ...
            logger$m.info('DEBUG ExecuteService - Running enhanced simulation:', {
            ...
            logger$m.error('Enhanced simulation failed, falling back to standard:', error);
...
class BridgeExecuteService {
    ...
    async bridgeAndExecute(params) {
        ...
        try {
            ...
            if (this.optimalBridgeAmount === '0' && !this.skipBridge) {
                logger$m.info('Simulation was not run, calculating optimal bridge amount...');
            ...
            if (this.skipBridge && execute) {
                logger$m.info(`Enhanced smart routing: Sufficient ${token} + gas balance on chain ${toChainId}, skipping bridge and executing directly`);
            ...
            logger$m.info(`Enhanced smart routing: Insufficient ${token} or gas balance on chain ${toChainId}, proceeding with bridge + execute`);
            ...
            logger$m.info('Bridge amount conversion for execution:', {
            ...
            logger$m.info('DEBUG bridgeAndExecute - Waiting for bridge settlement...');
            await new Promise((resolve) => setTimeout(resolve, 2000)); // 2 second delay
            logger$m.info('DEBUG bridgeAndExecute - Bridge settlement delay complete');
            ...
            (step) => this.adapter.nexusSDK.nexusEvents.emit(NEXUS_EVENTS.BRIDGE_EXECUTE_COMPLETED_STEPS, step), makeStep);
            const result = {
    ...
    async simulateBridgeAndExecute(params) {
        try {
            ...
            if (!this.skipBridge) {
                ...
                const { formatUnits } = await import('viem');
                const userFriendlyBridgeAmount = formatUnits(BigInt(this.optimalBridgeAmount), decimals);
                logger$m.info('Bridge amount conversion for simulation:', {
                    optimalBridgeAmountWei: this.optimalBridgeAmount,
                    userFriendlyBridgeAmount,
                    decimals,
                    token: params.token,
            ...
            if (execute) {
                try {
                    ...
                    if (bridgeReceiveAmount !== '0') {
                        ...
                        if (tokenDecimals) {
                            const receivedAmountBigInt = parseUnits$1(bridgeReceiveAmount, tokenDecimals);
                    ...
                    logger$m.warn(`Execute simulation error: ${simulationError}`);
                    executeSimulation = {
            ...
            if (totalBridgeFee !== '0' || executeSimulation?.gasUsed) {
                logger$m.debug('DEBUG bridge-execute-service - totalBridgeFee (ETH):', totalBridgeFee);
                logger$m.debug('DEBUG bridge-execute-service - executeSimulation?.gasUsed:', executeSimulation?.gasUsed);
                try {
                    const executeFee = executeSimulation?.gasCostEth || executeSimulation?.gasUsed || '0';
                    logger$m.debug('DEBUG bridge-execute-service - executeFee source value:', executeFee);
                    ...
                    if (executeSimulation?.gasCostEth === undefined) {
                        logger$m.debug('DEBUG bridge-execute-service - executeFee (gas units):', executeFee);
                        try {
                            ...
                            logger$m.warn('Failed to fetch gas price for execute fee conversion:', gpErr);
                        }
                    }
                    logger$m.debug('DEBUG bridge-execute-service - executeFee (ETH):', executeFeeEth);
                    ...
                    logger$m.debug('DEBUG bridge-execute-service - totalFeeEth:', totalFeeEth);
                    totalEstimatedCost = {
                        total: totalFeeEth,
                        breakdown: {
                    ...
                    logger$m.warn('Could not calculate total cost - cost breakdown may be incomplete:', error);
                }
            }
            ...
            if (!this.skipBridge && executeSimulation?.gasUsed) {
                ...
                if (finalOptimization.skipBridge && !this.skipBridge) {
                    ...
                    this.optimalBridgeAmount = '0';
                    logger$m.info('Updated bridge decision after gas validation: bridge can be skipped');
                }
            }
            logger$m.info(`Enhanced balance check result: skipBridge = ${this.skipBridge} for chain ${params.toChainId}`);
            ...
            if (this.skipBridge) {
                ...
                logger$m.info('Bridge will be skipped - using execute-only simulation result');
                return {
                    steps: finalSteps,
    ...
    async handleExecutePhase(execute, toChainId, bridgeToken, bridgeAmount, enableTransactionPolling, transactionTimeout, waitForReceipt, receiptTimeout, requiredConfirmations, emitStep, makeStep, approvalBufferBpsOverride) {
        ...
        try {
            ...
            logger$m.info('DEBUG handleExecutePhase - Bridge amount (micro-units):', bridgeAmount);
            logger$m.info('DEBUG handleExecutePhase - Bridge token:', bridgeToken);
            const { formatUnits } = await import('viem');
            const decimals = TOKEN_METADATA[bridgeToken]?.decimals || 18;
            const userFriendlyAmount = formatUnits(BigInt(bridgeAmount), decimals);
            logger$m.info('DEBUG handleExecutePhase - Amount conversion:', {
            ...
            logger$m.info('DEBUG handleExecutePhase - Execute params created with user-friendly amount:', {
                userFriendlyAmount,
            ...
            try {
                ...
                logger$m.info('DEBUG handleExecutePhase - User balance on destination chain:', {
                    chainId: toChainId,
                    token: bridgeToken,
                ...
                logger$m.warn('DEBUG handleExecutePhase - Could not check destination balance:', balanceError);
            }
            // Execute the target contract call - let execute service handle approval
            logger$m.info('DEBUG handleExecutePhase - Executing contract call with params:', {
                ...finalExecuteParams,
                toChainId,
            });
            ...
            if (waitForReceipt && executeResult.transactionHash) {
                logger$m.info('DEBUG handleExecutePhase - Checking transaction success for:', executeResult.transactionHash);
                ...
                if (!transactionCheck.success) {
                    logger$m.error('DEBUG handleExecutePhase - Transaction failed:', transactionCheck.error);
                ...
                logger$m.info('DEBUG handleExecutePhase - Transaction succeeded with gas used:', transactionCheck.gasUsed);
            ...
            logger$m.error('DEBUG handleExecutePhase - Execute error:', executeError);
            emitStep(makeStep('EX', 'execute', { error: executeError.message }));
            throw new Error(`Execute phase failed: ${extractErrorMessage(executeError, 'execute phase')}`);
        }
    }
    /**
    ...
    normalizeAmountToWei(amount, token) {
        try {
            ...
            logger$m.info('DEBUG normalizeAmountToWei - Input:', { amount: amountStr, token });
            // Handle edge cases
            ...
            logger$m.info('DEBUG normalizeAmountToWei - Token info:', {
                tokenUpper,
                decimals,
                tokenMetadata,
            });
            ...
            if (!amountStr.includes('.') && amountStr.length > 10) {
                logger$m.info('DEBUG normalizeAmountToWei - Already in wei format');
                return amountStr;
            }
            // Handle hex values
            if (amountStr.startsWith('0x')) {
                const result = BigInt(amountStr).toString();
                logger$m.info(`DEBUG normalizeAmountToWei - Hex conversion: ${result}`);
                return result;
            }
            // Handle decimal amounts (need conversion to wei)
            if (amountStr.includes('.')) {
                const result = parseUnits$1(amountStr, decimals).toString();
                logger$m.info(`DEBUG normalizeAmountToWei - Decimal conversion: ${amountStr} -> ${result}`);
                return result;
            }
            ...
            if (tokenUpper === 'USDC') {
                ...
                if (numValue < USDC_MICRO_UNITS_THRESHOLD) {
                    const result = parseUnits$1(amountStr, 6).toString();
                    logger$m.info(`DEBUG normalizeAmountToWei - USDC user amount conversion: ${amountStr} -> ${result}`);
                    return result;
                }
                else {
                    // Larger numbers are likely already in micro-USDC
                    logger$m.info('DEBUG normalizeAmountToWei - USDC already in micro format');
                    return amountStr;
                }
            }
            ...
            if (numValue < 1000 || (tokenMetadata?.decimals === 6 && numValue < 1000000)) {
                ...
                const result = parseUnits$1(amountStr, decimals).toString();
                logger$m.info(`DEBUG normalizeAmountToWei - User amount conversion: ${amountStr} -> ${result}`);
                return result;
            }
            else {
                // Assume larger numbers are already in the correct format
                logger$m.info('DEBUG normalizeAmountToWei - Already in correct format');
                return amountStr;
            }
        }
        catch (error) {
            // If conversion fails, return original
            logger$m.warn(`Failed to normalize amount ${amount} for token ${token}:`, error);
    ...
    async simulateFailedTransaction(txHash) {
        try {
            ...
            logger$m.info(`DEBUG simulateFailedTransaction - Simulating at block: ${simulationBlock}`);
            ...
            logger$m.warn('DEBUG simulateFailedTransaction - eth_call succeeded but original transaction failed. This might indicate a state-dependent failure.');
            return 'Transaction failed due to state changes or gas issues';
        }
        catch (error) {
            logger$m.info('DEBUG simulateFailedTransaction - eth_call failed as expected, extracting revert reason');
    ...
    async checkTransactionSuccess(txHash, chainId, maxRetries = 5, retryDelay = 3000) {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                logger$m.info(`DEBUG checkTransactionSuccess - Attempt ${attempt}/${maxRetries}: Checking transaction: ${txHash} on chain: ${chainId}`);
                // Ensure we're on the correct chain before checking transaction
                const currentChainId = await this.adapter.nexusSDK.getEVMClient().getChainId();
                if (currentChainId !== chainId) {
                    logger$m.info(`DEBUG checkTransactionSuccess - Switching from chain ${currentChainId} to ${chainId}`);
                    try {
                        ...
                        logger$m.error(`DEBUG checkTransactionSuccess - Failed to switch to chain ${chainId}:`, switchError);
                        return {
                            success: false,
                ...
                if (!receipt) {
                    if (attempt < maxRetries) {
                        logger$m.info(`DEBUG checkTransactionSuccess - Receipt not found, retrying in ${retryDelay}ms...`);
                ...
                logger$m.info(`DEBUG checkTransactionSuccess - Receipt status: ${receipt.status}`);
                ...
                if (receipt.status === 'success') {
                    ...
                    try {
                        ...
                        logger$m.warn('DEBUG checkTransactionSuccess - Simulation failed:', simError);
                        // Keep generic error message if simulation fails
                    }
                    logger$m.info(`DEBUG checkTransactionSuccess - Final error: ${errorMessage}`);
                    return {
                        success: false,
                        error: errorMessage,
                        gasUsed: toHex(receipt.gasUsed),
                ...
                logger$m.error(`DEBUG checkTransactionSuccess - Attempt ${attempt} failed:`, error);
                if (attempt < maxRetries) {
                    logger$m.info(`DEBUG checkTransactionSuccess - Retrying in ${retryDelay}ms...`);
                    await new Promise((resolve) => setTimeout(resolve, retryDelay));
                    continue; // Retry
                }
        ...
        try {
            ...
            if (destinationBalanceBigInt >= requiredAmountBigInt) {
                ...
                if (gasEstimate || gasCostEth) {
                    ...
                    if (!hasGasBalance) {
                        logger$m.info(`Insufficient gas balance on chain ${chainId}, cannot skip bridge`);
                        return { skipBridge: false, optimalAmount: requiredAmount };
                    }
                }
                logger$m.info(`Sufficient ${token} and gas balance on chain ${chainId}, bridge can be skipped`);
            ...
            logger$m.info(`Optimal bridge calculation:`, {
                token,
                chainId,
                requiredAmount,
                destinationBalance,
            ...
            logger$m.warn(`Failed to calculate optimal bridge amount: ${error}`);
    ...
    async getDestinationChainBalance(chainId, token) {
        try {
            logger$m.info(`Getting ${token} balance on chain ${chainId}`);
            ...
            const balanceInWei = parseUnits$1(chainBalance.balance, decimals);
            logger$m.info(`Balance found:`, {
                token,
                chainId,
            ...
            logger$m.warn(`Failed to get destination chain balance: ${error}`);
            return null;
        }
    }
    /**
    ...
    async checkGasBalance(chainId, gasEstimate, gasCostEth) {
        try {
            ...
            if (!chainMetadata) {
                logger$m.warn(`No chain metadata found for chain ${chainId}`);
                return false;
            }
            const nativeTokenSymbol = chainMetadata.nativeCurrency.symbol;
            logger$m.info(`Checking ${nativeTokenSymbol} balance on chain ${chainId} for gas`);
            // Get user's unified balances
            const balances = (await this.adapter.nexusSDK.getUnifiedBalances());
            ...
            if (!nativeTokenBalance || !nativeTokenBalance.breakdown) {
                logger$m.info(`No ${nativeTokenSymbol} balance found`);
                return false;
            }
            // Find balance on the specific chain
            const chainBalance = nativeTokenBalance.breakdown.find((balance) => balance.chain.id === chainId);
            if (!chainBalance) {
                logger$m.info(`No ${nativeTokenSymbol} balance found on chain ${chainId}`);
                return false;
            }
            ...
            if (gasCostEth) {
                ...
                try {
                    ...
                    logger$m.warn(`Failed to fetch gas price for gas balance check: ${error}`);
                    return false;
                }
            }
            ...
            logger$m.info(`Gas balance check result:`, {
                nativeTokenSymbol,
                chainId,
            ...
            logger$m.warn(`Failed to check gas balance: ${error}`);
            return false;
        }
    }
    /**
    ...
    async executeDirectly(execute, toChainId, token, amount, enableTransactionPolling, transactionTimeout, waitForReceipt, receiptTimeout, requiredConfirmations) {
        try {
            ...
            // Emit expected steps for execute-only flow
            this.adapter.nexusSDK.nexusEvents.emit(NEXUS_EVENTS.BRIDGE_EXECUTE_EXPECTED_STEPS, executeSteps);
            // Execute directly using existing funds
            const { executeTransactionHash, executeExplorerUrl, approvalTransactionHash } = await this.handleExecutePhase(execute, toChainId, token, amount, enableTransactionPolling, transactionTimeout, waitForReceipt, receiptTimeout, requiredConfirmations, (step) => this.adapter.nexusSDK.nexusEvents.emit(NEXUS_EVENTS.BRIDGE_EXECUTE_COMPLETED_STEPS, step), makeStep);
            return {
                executeTransactionHash,
                executeExplorerUrl,
                approvalTransactionHash,
...
class ChainAbstractionAdapter {
    constructor(nexusSDK) {
        ...
        logger$m.debug('ChainAbstractionAdapter', { nexusSDK });
        // Initialize services
...
class NexusSDK extends CA {
    constructor(config) {
        super(config);
        logger$m.debug('Nexus SDK initialized with config:', config);
