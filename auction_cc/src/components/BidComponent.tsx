...
const KEEPER_API_URL =
  process.env.NEXT_PUBLIC_KEEPER_API_URL || "http://localhost:3001";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (existingBid > 0) {
      if (existingBidChain !== null && selectedChain !== existingBidChain) {
        toast.error(
          `You can only bid from ${
            CHAIN_NAMES[existingBidChain as keyof typeof CHAIN_NAMES]
          }, the chain where you placed your first bid.`
        );
        return;
      }
      ...
      if (newTotalBidAmount <= existingBid) {
        toast.error(
          `Your new bid must be higher than your total existing bid of $${existingBid.toFixed(
            2
          )} (across all chains)`
        );
        return;
    ...
    if (highestBid === 0) {
      if (newTotalBidAmount < reservePriceNum) {
        toast.error(
          `First bid must be at least $${reservePriceNum.toFixed(
            2
          )} (reserve price)`
        );
        return;
      ...
      if (newTotalBidAmount > startingPriceNum) {
        toast.error(
          `Bid cannot exceed $${startingPriceNum.toFixed(2)} (maximum price)`
        );
        return;
      }
    } else {
      if (newTotalBidAmount <= highestBid) {
        toast.error(
          `Your bid must be higher than the current highest bid of $${highestBid.toFixed(
            2
          )}`
        );
        return;
    ...
    const rawIncrementalAmount =
      existingBid > 0 ? newTotalBidAmount - existingBid : newTotalBidAmount;
    const incrementalAmount = Math.round(rawIncrementalAmount * 1e6) / 1e6;

    if (incrementalAmount > unifiedBalance) {
      toast.error(
        `Insufficient balance. You need $${incrementalAmount.toFixed(
          2
        )} more, but only have $${unifiedBalance.toFixed(
          2
        )} ${selectedToken} available across all chains.`
      );
      return;
    }
    ...
    try {
      setLoading(true);
      const tokenAddress =
        TOKEN_ADDRESSES[selectedChain as keyof typeof TOKEN_ADDRESSES][
          selectedToken
        ];
      if (!tokenAddress) {
        throw new Error(
          `Token address not found for ${selectedToken} on chain ${selectedChain}`
        );
      }
      ...
      const bidManagerBalance = await tokenContract.balanceOf(
        BID_MANAGER_ADDRESS
      );
      ...
      if (bidManagerBalance < expectedAmount) {
        console.warn(
          "⚠️ Warning: BidManager balance is less than expected amount!"
        );
      ...
      const intentIdBytes32 = ethers.zeroPadValue(
        ethers.toBeHex(auctionId),
        32
      );
      ...
      <div className="bg-zinc-900 rounded-xl p-5 w-full max-w-2xl max-h-[85vh] overflow-y-auto border border-zinc-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
              ...
              {bidsLoading ? (
                ...
                <>
                  ...
                  <div className="text-lg font-bold text-white">
                    $
                    {parseFloat(ethers.formatUnits(reservePrice, 6)).toFixed(2)}{" "}
                    - $
                    {parseFloat(ethers.formatUnits(startingPrice, 6)).toFixed(
                      2
                    )}
                  </div>
                ...
                parseFloat(ethers.formatUnits(startingPrice, 6)) ? (
                <>
                  <div className="flex justify-between items-center mb-1">
          ...
          {existingBid > 0 && (
            <div className="bg-yellow-500/10 rounded-lg p-3 border border-yellow-500/30">
              ...
              <p className="text-xs text-zinc-500 mt-2">
                You can increase your bid. Only the additional amount will be
                transferred.
              </p>
            </div>
          )}
          ...
          <div className="bg-blue-500/10 rounded-lg p-3 border border-blue-500/30">
            ...
            <p className="text-xs text-zinc-500 mt-1">
              Unified {selectedToken} balance across Sepolia, Arbitrum, Base &
              Optimism
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-500">
              {highestBid >= parseFloat(ethers.formatUnits(startingPrice, 6))
                ...
                : `1 ${selectedToken} = $1.00 USD • Max: $${unifiedBalance.toFixed(
                    2
                  )}`}
            </p>
          </div>
          ...
          <div className="bg-black/50 rounded-lg p-3 border border-zinc-800">
                ...
                <>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-yellow-400 font-bold">
                      $
                      {amount
                        ? (parseFloat(amount) - existingBid).toFixed(2)
                        : "0.00"}{" "}
                      USD
                    </span>
                  </div>
            ...
            <div className="mt-2 pt-2 border-t border-zinc-700">
              <p className="text-xs text-zinc-500">
                💡{" "}
                {existingBid > 0
                  ? "Only the additional amount will be transferred from your wallet"
                  : "Nexus will automatically aggregate funds from all your chains"}
              </p>
            </div>
          </div>
